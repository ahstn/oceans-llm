...
MCP invocation logging, including Code Mode parent and nested child rows in `mcp_tool_invocations`, is observability, not spend. MCP and Code Mode executions write no ledger rows and run no budget checks; the usage ledger remains the only budget source. See [MCP Invocations](../../mcp/mcp-invocations.md).
