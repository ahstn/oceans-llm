...
`operation` is the public API family. Current values include `chat_completions`, `responses`, `embeddings`, and `decisions`.
...
Decisions requests record `operation: decisions`. Their `state`, questions, answers, provider attempt, and normalized usage follow the same payload, redaction, and byte-limit policy as other non-streaming inference requests.
