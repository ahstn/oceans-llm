...
| `AGENT_ANALYSIS_SHADOW_DIAGNOSTICS_ENABLED` | `false` | Let platform admins inspect session diagnostics while withholding the headline score. |
...
| `AGENT_ANALYSIS_TEAM_ADMIN_ENABLED` | `false` | Permit active team Owners/Admins to inspect their current team's sessions, but only when calibrated scores are also enabled. |
...
`mise run dev-stack` refreshes the local demo request and agent-session fixtures, enables shadow diagnostics by default, and starts the report worker. The **Observability → Agent Sessions** page becomes available to the seeded platform admin; reports populate as the queued demo analyses are processed. The curated data includes a ten-request Jira release workflow with raw file operations, six available Jira tools, and two direct MCP calls, plus a ten-request repository workflow with eight read, write, or edit calls. Set `AGENT_ANALYSIS_SHADOW_DIAGNOSTICS_ENABLED=false` before starting the stack to exercise the hidden-surface state.
...
Each displayed Agent Session is backed by an internal session window. Oceans first uses bounded, authenticated correlation evidence supplied by supported harnesses. Known session headers or payload-policy-permitted metadata are normalized and stored only within the authenticated ownership and harness scope. Disabled and summary-only payload policies permit known session headers but leave body-derived observations unavailable. Requests without sufficient session evidence still form harness-specific, owner-scoped windows instead of receiving a fabricated session identifier.

An open session is reused only within the same ownership scope, harness, explicit request-tag set, and compatible observed session. Model, operation, and caller class remain report dimensions but do not split a semantic session when an agent switches models or request modes. The boundary dimensions are represented by a stable opaque key; raw prompt or response content is never part of the grouping key. Passive correlation is dispatched after request logging and does not delay the model response. Idle sessions are finalized after a versioned 30-minute gap, then queued for analysis. A session link stores a determinate success/failure outcome or an explicit unknown outcome for partial usage-only failures independently of request-log retention. Late evidence invalidates the current report and queues a new immutable report version; an older in-flight worker cannot publish after the session watermark advances or complete work owned by a newer lease.
...
- **Cohort** selects prior successful, version-compatible sessions in a fixed fallback cascade: exact boundary, same harness/model/operation/caller, same harness/model, then same harness. Every level requires at least six peers. The chosen level, sample size, and immutable snapshot digest remain visible; otherwise the score stays unavailable.
- **Limitations** are first-class output. Missing facts are unavailable; they are not converted to neutral or zero values.
- **Versions and identity** expose the immutable analysis ID, input watermark, observation-set ID, cohort-snapshot digest, report schema, analyzer, score policy, pricing policy, observation parser, and session-boundary policy needed to reproduce an interpretation.
...
Request links and inferred observations follow request-log-aligned retention. A minimal session shell remains while an immutable report references it, so reports stay queryable until their independent expiry, which defaults to 90 days. Reports are deleted when their owner is deleted. Completed or terminally failed queue rows default to seven days. An hourly retention loop enforces both independent windows even when passive analysis is disabled; both are configurable with the runtime variables above.
