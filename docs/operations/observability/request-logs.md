...
`See also`: [Observability and Request Logs](../observability-and-request-logs.md), [MCP Invocations](../../mcp/mcp-invocations.md), [MCP Registry and Discovery](../../contributing/mcp/mcp-registry-and-discovery.md), [Tagging](../tagging.md), [Request Lifecycle and Failure Modes](../../reference/request-lifecycle-and-failure-modes.md), [Data Relationships](../../contributing/reference/data-relationships.md), [Admin Control Plane](../../access/admin-control-plane.md)
...
The gateway splits request-log data across:
...
Registry-backed execution uses stable external MCP server/tool ids for invocation records. The gateway stores MCP token-overhead summaries in `request_mcp_token_overheads` by `request_id` and optional `request_log_id`; they describe context-window pressure, not spend.
...
- MCP per-tool invocation records: [MCP Invocations](../../mcp/mcp-invocations.md)
