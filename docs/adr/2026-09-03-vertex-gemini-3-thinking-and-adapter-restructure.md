...
2. Map OpenAI fields in a single pass over `request.extra`: sampling keys into `generationConfig`, `reasoning_effort` / `reasoning.effort` into `thinkingConfig` (`thinkingLevel` for 3.x, `thinkingBudget` for 2.5, `includeThoughts: true`), `response_format` into `responseMimeType` / `responseJsonSchema`, `max_completion_tokens` aliased to `maxOutputTokens`, and OpenAI-only keys dropped. Caller `generationConfig` deep-merges over the mapped keys. Conflicts (two different max-token values, `reasoning_effort` alongside a caller `thinkingConfig`) are typed errors.
   - `"none"` / `"off"` sends `thinkingBudget: 0` where the model allows it (2.5 Flash). Gemini 3.x and 2.5 Pro cannot disable thinking, so the adapter sends the lowest accepted level or budget with `includeThoughts: false`: thought text is hidden from the response but the model still thinks and bills `reasoning_tokens`.
   - `GeminiModel` encodes the Gemini 3 conventions from the official model guides: `MINIMAL` exists only on Flash / Flash-Lite before 3.7 (later models and Pro start at `LOW`); from 3.6, `temperature`/`topP`/`topK` are ignored upstream and dropped, and `presencePenalty`/`frequencyPenalty`/`candidateCount` are rejected unless they carry the no-op default. The check runs on the merged `generationConfig` after caller config and route `extra_body` are applied. The client-config `ThinkingPolicy::GeminiLevel { supports_minimal }` mirrors the `MINIMAL` rule so Pi and OpenCode never offer `minimal` on 3.7+.
...
6. Move `anthropic-beta` values from provider default headers and route headers into the `anthropic_beta` body array for `rawPredict`, appending `effort-2025-11-24` when `output_config.effort` is used with manual thinking.
   - The Anthropic-on-Vertex mapper now shares `AnthropicRequestOptions` with the direct Anthropic adapter, including its `max_tokens: 4096` default when the caller omits a limit. The previous Vertex-only default of `1024` truncated most tool-using Claude turns; the change is documented in `docs/providers/gcp-vertex.md`.
7. Derive `api_host` from `location` when unset (`global`, `us`/`eu` multi-region, regional). Batch text-embedding `predict` calls at 250 instances (one for `gemini-embedding-001`, which Vertex limits to a single input per request) and a 20,000-token aggregate budget bounded locally at one token per UTF-8 byte (the tokenizer's byte-fallback ceiling); validate the prediction count per batch.
