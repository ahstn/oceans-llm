...
- `mcp_tool_invocations`
  - Key columns: `mcp_tool_invocation_id`, `parent_invocation_id`, `request_log_id`, `request_id`, `api_key_id`, `user_id`, `team_id`, `owner_kind`, `server_id`, `server_display_key`, `tool_id`, `tool_display_key`, `status`, `policy_result`, `latency_ms`, `error_code`, `has_payload`, `arguments_payload_truncated`, `result_payload_truncated`, `arguments_payload_redacted`, `result_payload_redacted`, `metadata_json`, `occurred_at`
  - Notes: MCP invocation rows are correlated by `request_id`; `request_log_id` is nullable and non-owning because request-log summaries are written at final outcome and may be absent or purged independently. Server/tool stable IDs are nullable until registry records exist, but display keys are required. `parent_invocation_id` is a nullable self-reference with `ON DELETE SET NULL`: Code Mode `explore`/`execute` executions write a parent row under the synthetic `code-mode` server identity, and nested tool calls made from the sandbox link back to it. Backend caveat: `ON DELETE SET NULL` is enforced on Postgres only — libsql connections do not enable the `foreign_keys` pragma, so SQLite-backed deployments would leave dangling parent ids if rows were ever deleted (nothing deletes `mcp_tool_invocations` today).
...
- `mcp_aggregate_sessions`
  - Key columns: `session_id`, `token_hash`, `surface`, `api_key_id`, `owner_kind`, `owner_user_id`, `owner_team_id`, `owner_service_account_id`, `protocol_version`, `initialized`, `expires_at`, `created_at`, `updated_at`, `revoked_at`
  - Notes: durable Streamable HTTP transport state shared by the gateway-owned MCP surfaces. Only token hashes are stored. Sessions are bound to the authenticated API key and owner metadata, and to the surface they were initialized against (`surface` is `aggregate` for `/mcp` or `code_mode` for `/code-mode-mcp`); reuse by another principal or at the other surface is treated as not found.
