...
describe.skipIf(!cacheCanaryModel || !process.env.GATEWAY_BASE_URL)(
  "Responses prompt-cache canary",
  () => {
    ...
    test("records a first-turn write and a second-turn read on one route", async () => {
      ...
      const first = await sendResponse(
        runtime.baseUrl,
        runtime.apiKey,
        model,
        cacheKey,
      ...
      const second = await sendResponse(
        runtime.baseUrl,
        runtime.apiKey,
        model,
        cacheKey,
...
function successfulAttempt(attempts: RequestLogDetail["attempts"]) {
