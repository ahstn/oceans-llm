...
pub async fn v1_chat_completions(
    ...
    tracing::info!(
        ...
        resolved_model = %resolved.execution_model.model_key,
    ...
    if request.stream || !allow_fallback {
        ...
        let context = build_provider_context(
            &request_id,
            &resolved.requested_model.model_key,
            &route,
        ...
        if request.stream {
            let stream = match provider.chat_completions_stream(&request, &context).await {
                ...
                Err(error) => {
                    ...
                    best_effort_log_request(
                        ...
                        &request_id,
                        &resolved.requested_model.model_key,
                        &resolved.execution_model.model_key,
            ...
            let body_stream = wrap_stream_with_request_logging(LoggingBodyStreamState {
                ...
                model_key: resolved.requested_model.model_key.clone(),
                resolved_model_key: resolved.execution_model.model_key.clone(),
        ...
        let value = match value {
            Ok(value) => normalize_response_model(value, &resolved.requested_model.model_key),
            Err(error) => {
                best_effort_log_request(
                    &state.service,
                    &auth,
...
async fn best_effort_log_request(
    ...
    let metadata = request_log_metadata(summary.attempt_count, summary.stream, resolved_model_key);
...
fn request_log_metadata(
    ...
    metadata.insert(
        ...
        Value::String(resolved_model_key.to_string()),
    );
    metadata
}
