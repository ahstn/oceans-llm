...
use gateway_mcp::{
    ...
    server::{
        ...
        McpServerMessage, call_tool_result, initialize_result, json_rpc_error, json_rpc_success,
        parse_client_message, tools_list_result,
    },
};
use gateway_service::{DescribeMcpToolInput, MAX_SEARCH_LIMIT, McpCatalog, SearchMcpToolsInput};
...
fn list_builtin_tools(id: JsonRpcId) -> Response<Body> {
    let tools = vec![search_tools_definition(), describe_tool_definition()];
    json_rpc_response(
        StatusCode::OK,
        ...
        _ => {
            return json_rpc_response(
                ...
                json_rpc_error(
                    ...
                    "MCP aggregate gateway exposes only search_tools and describe_tool",
                ),
                None,
            );
        }
...
mod tests {
    ...
    fn aggregate_tools_list_contains_only_discovery_tools() {
        let names = [search_tools_definition(), describe_tool_definition()]
            .into_iter()
            .map(|tool| tool.name)
            .collect::<Vec<_>>();
        assert_eq!(names, vec!["search_tools", "describe_tool"]);
    }
}
