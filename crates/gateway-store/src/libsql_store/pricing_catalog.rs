    ...
    async fn apply_model_pricing_sync(
        ...
        for update in &changes.update_provenance {
            tx.execute(
                ...
                "#,
                libsql::params![
                    update.model_pricing_id.to_string(),
                    update.provenance.source.as_str(),
                    update.provenance.etag.as_deref(),
                    update.provenance.fetched_at.unix_timestamp(),
                    update.updated_at.unix_timestamp()
                ],
            )
            .await
            .map_err(to_write_error)?;
