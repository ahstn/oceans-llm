...
WITH ranked AS (
    ...
    FROM budgets WHERE scope_key IN (SELECT value FROM json_each(?1))
)
SELECT budget_id, scope_kind, scope_key, user_id, service_account_id, model_id,
           upstream_model, cadence, amount_10000, hard_limit, timezone, is_active,
           created_at, updated_at, source_kind, source_key
...
const UPSERT: &str = r#"
WITH input AS (
    SELECT json_extract(value, '$.scope_key') AS scope_key,
           json_extract(value, '$.scope_kind') AS scope_kind,
           json_extract(value, '$.user_id') AS user_id,
           json_extract(value, '$.service_account_id') AS service_account_id,
           json_extract(value, '$.model_id') AS model_id,
           json_extract(value, '$.upstream_model') AS upstream_model,
           json_extract(value, '$.budget_id') AS budget_id,
           json_extract(value, '$.cadence') AS cadence,
           json_extract(value, '$.amount_10000') AS amount_10000,
           json_extract(value, '$.hard_limit') AS hard_limit,
           json_extract(value, '$.timezone') AS timezone,
           json_extract(value, '$.source_kind') AS source_kind,
           json_extract(value, '$.source_key') AS source_key,
           json_extract(value, '$.expected_kind') AS expected_kind,
           json_extract(value, '$.expected_key') AS expected_key,
...
WITH input AS (
    SELECT json_extract(value, '$.budget_id') AS budget_id,
           json_extract(value, '$.source_kind') AS source_kind,
           json_extract(value, '$.source_key') AS source_key
    FROM json_each(?1)
)
...
WITH input AS (
    SELECT json_extract(value, '$.scope_key') AS scope_key,
           ...
           json_extract(value, '$.model_id') AS model_id,
           json_extract(value, '$.upstream_model') AS upstream_model,
           json_extract(value, '$.window_start') AS window_start,
           json_extract(value, '$.window_end') AS window_end
    FROM json_each(?1)
)
SELECT i.scope_key, COALESCE(SUM(u.computed_cost_10000), 0) AS total
...
pub(super) async fn upsert_active_budgets_with_source_guard(
    ...
    store
        .connection
        .execute(UPSERT, libsql::params![input])
        .await
...
pub(super) async fn deactivate_budgets_by_source(
    ...
    store
        ...
        .execute(
            DEACTIVATE,
            libsql::params![input, updated_at.unix_timestamp()],
        )
        .await
        .map_err(to_write_error)?;
    Ok(())
...
pub(super) async fn sum_usage_cost_by_budget_scope(
    ...
    while let Some(row) = rows.next().await.map_err(to_query_error)? {
        totals.insert(
            ...
            Money4::from_scaled(row.get(1).map_err(to_query_error)?),
        );
    }
    Ok(totals)
}
...
impl LibsqlStore {
    pub async fn list_budget_contacts(
        ...
        while let Some(row) = rows.next().await.map_err(to_query_error)? {
            contacts.push(gateway_core::BudgetContact {
                user_id: crate::shared::parse_uuid(&row.get::<String>(0).map_err(to_query_error)?)?,
                email: row.get(1).map_err(to_query_error)?,
                alert_team_id: row
                    .get::<Option<String>>(2)
                    .map_err(to_query_error)?
                    .as_deref()
