...
impl BudgetRepository for PostgresStore {
        ...
        let rows = sqlx::query(
            ...
            SELECT * FROM (
                SELECT
                    ...
                    COALESCE(SUM(u.uncached_input_tokens), 0)::BIGINT,
                ...
                WHERE u.occurred_at >= $1
                  AND u.occurred_at < $2
                  AND ($3::TEXT IS NULL OR $3::TEXT = 'user')
                ...
                WHERE u.occurred_at >= $1
                  AND u.occurred_at < $2
                  AND ($3::TEXT IS NULL OR $3::TEXT = 'service_account')
                  AND ($4::TEXT IS NULL OR u.user_id = $4)
    ...
    async fn list_usage_model_token_daily_aggregates(
        ...
        let rows = sqlx::query(
            ...
            WHERE u.occurred_at >= $1
              ...
              AND (
                $3::TEXT IS NULL
                OR ($3::TEXT = 'user' AND u.user_id IS NOT NULL)
                OR ($3::TEXT = 'service_account' AND u.service_account_id IS NOT NULL)
              )
              AND ($4::TEXT IS NULL OR u.user_id = $4)
    ...
    async fn list_usage_user_leaderboard(
        ...
        let mut output = Vec::with_capacity(rows.len());
        for row in rows {
            let harness_key: Option<String> = row.try_get(5).map_err(to_query_error)?;
            let harness_label: Option<String> = row.try_get(6).map_err(to_query_error)?;
            output.push(UsageLeaderboardUserRecord {
                ...
                tool_cardinality_averages: gateway_core::RequestToolCardinalityAverages {
                    referenced_mcp_server_count: row.try_get(7).map_err(to_query_error)?,
                    exposed_tool_count: row.try_get(8).map_err(to_query_error)?,
                    invoked_tool_count: row.try_get(9).map_err(to_query_error)?,
                    filtered_tool_count: row.try_get(10).map_err(to_query_error)?,
                },
            });
        }
    ...
    async fn insert_usage_ledger_if_absent(
        ...
        .bind(event.model_route_id.map(|value| value.to_string()))
...
fn token_usage_buckets_from_row(
    ...
    Ok(TokenUsageBuckets {
        request_count: row.try_get(offset).map_err(to_query_error)?,
        input_tokens: row.try_get(offset + 1).map_err(to_query_error)?,
        output_tokens: row.try_get(offset + 2).map_err(to_query_error)?,
        uncached_input_tokens: row.try_get(offset + 3).map_err(to_query_error)?,
        cache_read_tokens: row.try_get(offset + 4).map_err(to_query_error)?,
        cache_write_tokens: row.try_get(offset + 5).map_err(to_query_error)?,
    })
}
