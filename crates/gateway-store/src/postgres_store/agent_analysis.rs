...
fn decode_session_source(row: &PgRow) -> Result<AgentSessionSourceRecord, StoreError> {
    Ok(AgentSessionSourceRecord {
        agent_session_source_id: parse_uuid(&row.try_get::<String, _>(0).map_err(to_query_error)?)?,
...
fn decode_session(row: &PgRow) -> Result<AgentSessionRecord, StoreError> {
    ...
    Ok(AgentSessionRecord {
        ...
        agent_session_source_id: parse_optional_uuid(row.try_get(1).map_err(to_query_error)?)?,
...
fn decode_request_link(row: &PgRow) -> Result<AgentSessionRequestLinkRecord, StoreError> {
...
impl PostgresStore {
    ...
    async fn load_observation_set(
        ...
        let row = sqlx::query("SELECT observation_set_id, agent_session_id, parser_version, source_watermark_at, coverage_json, created_at FROM agent_inferred_observation_sets WHERE agent_session_id = $1 ORDER BY source_watermark_at DESC, created_at DESC LIMIT 1")
            .bind(agent_session_id.to_string()).fetch_optional(&self.pool).await.map_err(to_query_error)?;
    ...
    async fn query_observation_set_by_id(
        ...
        let row = sqlx::query("SELECT observation_set_id, agent_session_id, parser_version, source_watermark_at, coverage_json, created_at FROM agent_inferred_observation_sets WHERE observation_set_id = $1")
