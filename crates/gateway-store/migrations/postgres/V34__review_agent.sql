CREATE TABLE IF NOT EXISTS review_agent_repositories (
  ...
  inline_review_enabled BIGINT NOT NULL DEFAULT 1 CHECK (inline_review_enabled IN (0, 1)),
  pr_summary_enabled BIGINT NOT NULL DEFAULT 1 CHECK (pr_summary_enabled IN (0, 1)),
  diagrams_enabled BIGINT NOT NULL DEFAULT 0 CHECK (diagrams_enabled IN (0, 1)),
  linked_issue_detection_enabled BIGINT NOT NULL DEFAULT 1 CHECK (linked_issue_detection_enabled IN (0, 1)),
  linked_issue_assessment_enabled BIGINT NOT NULL DEFAULT 0 CHECK (linked_issue_assessment_enabled IN (0, 1)),
  default_model_key TEXT,
  max_inline_comments BIGINT CHECK (max_inline_comments IS NULL OR max_inline_comments >= 0),
  request_changes_on_high_severity BIGINT NOT NULL DEFAULT 0 CHECK (request_changes_on_high_severity IN (0, 1)),
...
CREATE TABLE IF NOT EXISTS review_agent_pull_requests (
  ...
  is_draft BIGINT NOT NULL DEFAULT 0 CHECK (is_draft IN (0, 1)),
  created_at BIGINT NOT NULL,
  updated_at BIGINT NOT NULL,
...
CREATE TABLE IF NOT EXISTS review_agent_runs (
  ...
  head_sha TEXT,
  github_run_id TEXT,
  github_run_attempt BIGINT CHECK (github_run_attempt IS NULL OR github_run_attempt > 0),
  status TEXT NOT NULL CHECK (status IN ('queued', 'in_progress', 'succeeded', 'failed', 'cancelled', 'skipped')),
  started_at BIGINT,
  heartbeat_at BIGINT,
  finished_at BIGINT,
  duration_ms BIGINT CHECK (duration_ms IS NULL OR duration_ms >= 0),
  files_changed BIGINT CHECK (files_changed IS NULL OR files_changed >= 0),
  additions BIGINT CHECK (additions IS NULL OR additions >= 0),
  deletions BIGINT CHECK (deletions IS NULL OR deletions >= 0),
  changed_loc BIGINT CHECK (changed_loc IS NULL OR changed_loc >= 0),
  inline_comments_created BIGINT CHECK (inline_comments_created IS NULL OR inline_comments_created >= 0),
  inline_comments_updated BIGINT CHECK (inline_comments_updated IS NULL OR inline_comments_updated >= 0),
  inline_comments_skipped BIGINT CHECK (inline_comments_skipped IS NULL OR inline_comments_skipped >= 0),
  inline_comments_failed BIGINT CHECK (inline_comments_failed IS NULL OR inline_comments_failed >= 0),
  stale_comments_deleted BIGINT CHECK (stale_comments_deleted IS NULL OR stale_comments_deleted >= 0),
  ...
  summary_status TEXT,
  diagram_status TEXT,
  linked_issue_count BIGINT CHECK (linked_issue_count IS NULL OR linked_issue_count >= 0),
