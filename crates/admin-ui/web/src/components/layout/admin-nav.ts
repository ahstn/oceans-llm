...
export function getAdminNavSections(globalRole: string, agentAnalysisEnabled = false) {
  return adminNavSections
    .map((section) => ({
      ...section,
      items: section.items.filter((item) => {
        if (item.to === '/observability/agent-sessions') return agentAnalysisEnabled
        return globalRole === 'platform_admin' || !item.adminOnly
      }),
    }))
