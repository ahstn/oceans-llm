  ...
  return (
    <>
      {tokens.map((token, index) => {
        ...
        if (!style && !token.word) {
          // Tokens have no stable identity; their ordered position defines the rendered slice.
 ...
 */
// Keep line state, selection, folding, and inline actions in one memoized render boundary.
 ...
 */
// This registry primitive coordinates highlighting, streaming, folding, and selection state.
// oxlint-disable-next-line eslint/max-lines-per-function
function CodeBlock({
  code,
  language,
...
// Keep viewport, keyboard, selection, folding, and scrolling behavior in one surface.
// oxlint-disable-next-line eslint/max-lines-per-function
    ...
    >
      ...
      <div
        ...
        aria-activedescendant={
          selectable && activeLine !== null ? `${contentId}-L${activeLine}` : undefined
        }
        tabIndex={0}
...
function CodeBlockExpandButton({
  ...
  const { expanded, setExpanded, collapsible, contentId } =
    useInternalConfig('CodeBlockExpandButton')
