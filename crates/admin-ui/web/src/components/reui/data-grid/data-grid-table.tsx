// ReUI registry source: preserve its shared render and resize workflows for upstream updates.
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <AppIcon data-icon="inline-start" icon={PlusSignIcon} aria-hidden={true} />
        ...
        className: cn(
          ...
          isPinned === 'end'
            ? cn(
                ...
                props.tableLayout?.columnsPinnable
                  ? 'before:hidden'
                  : 'before:bg-border before:absolute before:inset-y-0 before:start-0 before:w-px',
              )
            : isLastVisibleColumn
              ...
              : isPinned
                ? cn(
                    ...
                    props.tableLayout?.columnsPinnable
                      ? 'before:hidden'
                      : 'before:bg-border before:absolute before:inset-y-0 before:end-0 before:w-px',
                  )
                : cn(
...
function DataGridTableFootRow({ children }: { children: ReactNode }) {
  ...
  return (
    <tr
      ...
      className={cn(
        props.tableLayout?.footerBackground && 'bg-muted/40',
  ...
  return (
    <tr
      ...
      className={cn(
        ...
        props.getRowStatus &&
          'data-[row-status=deleted]:bg-destructive/5 data-[row-status=dirty]:bg-warning/5 data-[row-status=new]:bg-success/5 data-[row-status=deleted]:opacity-60 data-[row-status=deleted]:[&_td]:line-through',
        props.getRowStatus &&
          'data-[row-status=deleted]:[&>td[data-pinned]]:bg-[color-mix(in_oklab,var(--destructive)_5%,var(--background))] data-[row-status=dirty]:[&>td[data-pinned]]:bg-[color-mix(in_oklab,var(--color-warning)_5%,var(--background))] data-[row-status=new]:[&>td[data-pinned]]:bg-[color-mix(in_oklab,var(--success)_5%,var(--background))]',
        ...
        pinnedBoundary === 'top' &&
          '[&>td]:shadow-[0_2px_0_color-mix(in_oklab,var(--border)_40%,transparent)]',
        // A bottom-pinned row borders scrolled content along its TOP edge,
        // so its separator shadow casts upward.
        pinnedBoundary === 'bottom' &&
          '[&>td]:shadow-[0_-2px_0_color-mix(in_oklab,var(--border)_40%,transparent)]',
    ...
    >
      {row.getCanExpand() ? (
        <button
          ...
          className={cn(
            'text-muted-foreground hover:text-foreground inline-flex items-center justify-center rounded-lg transition-colors',
            controlSize,
          )}
        >
          {children ?? (
...
function DataGridTableBodyRows<TData extends object>({
  ...
  if (isLoading && props.loadingMode === 'skeleton' && pagination?.pageSize) {
    ...
    return (
      <>
        {Array.from({ length: pagination.pageSize }, (_, index) => `skeleton-${index}`).map(
          (rowKey) => (
