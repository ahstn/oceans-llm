...
export function readReviewResult(path: string): ReviewResult {
  ...
  const findings = Array.isArray(parsed.findings)
    ? parsed.findings.map(sanitizeFinding).filter(Boolean)
    : []
...
function sanitizeFinding(input: any): Finding | undefined {
  ...
  const startLine =
    Number.isInteger(input.start_line) && input.start_line > 0 ? input.start_line : undefined
  const endLine =
    Number.isInteger(input.end_line) && input.end_line >= input.line ? input.end_line : undefined
  return {
    path: input.path,
    line: input.line,
