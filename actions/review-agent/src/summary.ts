...
export class GitHubPublisher implements Publisher {
  ...
  async publish(input: PublishInput): Promise<RunMetrics> {
    ...
    if (input.inlineReview) {
      ...
      const event =
        input.requestChangesOnHighSeverity && input.result.findings.some(isHighSeverity)
          ? 'REQUEST_CHANGES'
          : 'COMMENT'
...
async function findManagedComment(octokit: any, input: PublishInput): Promise<any> {
  for (let page = 1; ; page += 1) {
    // Pagination stops at the first match or final page; the next request depends on this response.
