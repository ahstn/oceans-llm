...
- `agent-harnesses.omp` — Seeded `omp/` traffic is labeled Oh My Pi and remains text-only.
...
4. Find the API rows keyed `mastra` and `oh_my_pi`. The Mastra table row must contain `[data-agent-harness-icon="mastra"]`; the Oh My Pi row must contain no `[data-agent-harness-icon]` descendant.
...
- Oh My Pi deliberately has no Lobe icon. Any icon in its row is a failure, including the unrelated Pi icon.
